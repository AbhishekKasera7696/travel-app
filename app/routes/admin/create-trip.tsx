...
export const loader = async () => {
    const response = await fetch("https://restcountries.com/v3.1/all?fields=name,flags,region,capital,population,latlng,maps"
    );
    const data = await response.json();

    return data.map((country: any) => ({
        name: country.flags.png + country.name.common,
...
const CreateTrip = ({ loaderData }: Route.ComponentProps ) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response.ok) {
                // try JSON body, else fallback to text
            ...
            const result: CreateTripResponse = await response.json().catch(() => ({ id: null }));
